...
class OrderRepository extends BaseRepository {
  ...
  // Get orders for a public.users user_id (or resolve from auth_id if needed).
  Future<List<Map<String, dynamic>>> getUserOrders(String userOrAuthId) async {
    try {
      ...
      final byUserId = await client
          .from('users')
          .select('user_id')
          .eq('user_id', userOrAuthId)
          .maybeSingle();
      if (byUserId != null) {
        userIdForQuery = byUserId['user_id'] as String?;
      } else {
        final byAuthId = await client
            .from('users')
            .select('user_id')
            .eq('auth_id', userOrAuthId)
            .maybeSingle();
        userIdForQuery = byAuthId?['user_id'] as String?;
      }

      if (userIdForQuery == null) {
        print('OrderRepository: No user found for id: $userOrAuthId');
        return [];
      }

      print('OrderRepository: Fetching orders for user_id: $userIdForQuery');

      // Get orders (status is a text column on orders, not a FK join).
